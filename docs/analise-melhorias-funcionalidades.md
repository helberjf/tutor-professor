...
Data: 2026-09-11 (revisado em 2026-09-12, quando o produto deixou de ser
voltado a crianças e passou a servir qualquer pessoa que queira estudar).
Base: leitura do código (`apps/api`, `apps/web`), do banco de
...
| 2 | **Modo estudo com PIN para a área da conta** | Hoje a mesma sessão abre a área da conta, onde dá para trocar a chave de IA, apagar estudantes e excluir a conta. | Alto | P |
...
## 0. Reposicionamento: o app é para qualquer pessoa — feito em 2026-09-12

- **Observado:** o produto nasceu como app infantil e dizia isso em todo lugar —
  "Área dos pais" na navegação, "Perfil da criança" nas configurações, um ícone de
  bebê, tokens `kid-*` no CSS, e, o que mais pesava, prompts que pediam ao modelo
  conteúdo "child-safe", "for a child", "children's picture-book". Um adulto que
  se cadastrasse recebia lições escritas para uma criança de sete anos.
- **Feito:**
  - Texto e rótulos: "criança" virou "estudante", `/parents` virou `/account`
    ("Área da conta", com redirect do link antigo para não quebrar favorito), e os
    tokens de CSS `kid-surface`/`kid-button`/`kid-*` viraram `app-*`/`brand-*`.
  - Faixa de idade de verdade: o perfil oferece 4-6, 7-9, 10-12, 13-17 e 18+, e o
    padrão de um perfil novo é adulto.
  - `services/audience.py` é o único lugar que decide o público de uma geração.
    Todo gerador (lição, livro, questões) lê dali, então **um adulto recebe
    exemplos de adulto e um perfil de criança continua recebendo as regras de
    conteúdo que uma criança precisa**. Tirar o assunto do prompt teria sido a
    mudança fácil e errada.
  - Rascunhos de privacidade e termos reescritos para uma conta que pode estudar
    sozinha, mantendo todas as cláusulas de menor de idade para quem cria esse
    tipo de perfil. Continuam precisando de advogado, como já diziam.
- **Pendente de decisão sua:** o banco ainda chama a entidade de `ChildProfile`
  e as rotas de `/api/parent/*`. Nada disso aparece para quem usa, e renomear
  custa migration mais 130 rotas — está anotado, não feito.

## 1. Experiência do estudante

### 1.0 Sessão única de estudo — feito em 2026-09-11
- **Observado:** para chegar à primeira questão o estudante passava por três telas
...
- **Feito:**
  ...
  3. **Fila de reposição em segundo plano:** `POST /api/study/questions/prefetch`
     ...
     já ter voltado. O estudante nunca espera o provedor.
  4. O cache compartilhado de lições geradas já existia (`auto_generate_lesson_for_child`
     reaproveita a lição do nível e materializa as perguntas por estudante).
...
- **Observado:** há TTS (Kokoro, com fallback do navegador), mas nenhum uso de
  `SpeechRecognition` ou `MediaRecorder` no front. O estudante ouve, mas nunca fala.
- **Proposta:** botão "Fale a palavra" na lição e na revisão, usando a Web Speech
  ...
  o que evita a questão de dado de voz de estudante.
...
- **Proposta:** chat com o modelo já configurado na conta, usando como contexto a
  ...
  - respostas curtas e no nível do estudante;
...
- **Proposta:**
  ...
  - **Proteção de sequência:** 1 dia de folga por semana, para o estudante não
    desanimar ao perder um dia.
  - Sem ranking entre estudantes (a auditoria já apontou isso como inadequado).
...
- **Proposta:** migrar `ReviewItem` e `LessonQuestion` para o FSRS, com os
  parâmetros ajustados para sessões curtas de estudante. É um algoritmo só para
...
- **Feito:** `/onboarding` em 3 passos — nome e idade, idioma, e um teste de
  ...
  primeira leitura. A área da conta devolve ao automático quando quiser.
  O primeiro login cai em `/onboarding`; um `?next=` explícito sempre ganha.
- **Impacto:** alto (ativação). **Esforço:** P.

---

## 2. Conta e responsáveis

### 2.1 Modo estudo com PIN
- **Observado:** nenhum PIN ou modo estudante no modelo (`ChildProfile`,
  `ParentSettings`) nem nas rotas. A área da conta (`/parents`) abre na mesma sessão
  em que o estudante estuda.
- **Proposta:** PIN de 4 dígitos (guardado com hash, com o mesmo freio de
  ...
  destrutivas. A navegação do estudante esconde "Área da conta" até o PIN.
...
- **Proposta:** e-mail de domingo por estudante com dias estudados, minutos,
...
- **Observado:** na aba English a meta do dia só fechava com texto escrito
  ...
  de adulto no caminho de um estudante que já tinha feito o trabalho.
...
- **Proposta:** limite diário opcional por estudante e horário permitido (ex.: não
...
- **Observado:** o conteúdo gerado passa por validação de formato, mas vai
  direto para o estudante. A auditoria de maio já recomendava curadoria.
...
- **Feito em 2026-09-11:** `GET /api/exams/sources` lista toda matéria do estudante
...
- **Continua valendo:** separar público infantil e adulto para programação e
  LeetCode, com um tipo de perfil na criação ("estudante" ou "estudante/adulto").
...
**Agora (1–2 semanas)**
3. PIN da área da conta (2.1). Precisa de migration (a próxima livre é a `0026`).
...
13. Perfil estudante × adulto completo (3.1) e quebra do `main.py` em routers.
