...
import { ArrowLeft, BarChart3, BookOpen, Bot, GraduationCap, KeyRound, Save, ShieldCheck, Sparkles, UserPlus, Users, Volume2 } from 'lucide-react';
...
// the language being studied above.
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      setForm({
        ...
        base_language: activeSettings.base_language ?? 'Portuguese',
      });
      setIsLoggedIn(true);
      setError(null);
    } catch (err) {
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm({
        ...
        base_language: settings.base_language ?? 'Portuguese',
      });
  ...
  async function handleCreateChild(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const child = await api.createParentChild({
        ...
        base_language: form.base_language,
      });
  ...
  return (
    <main className="min-h-screen px-3 py-5 sm:px-4 sm:py-6 md:px-10 md:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="app-surface mb-6 border-emerald-200 p-5 md:p-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr,0.85fr]">
            ...
            <div className="mt-8 grid gap-8">
              <section>
                ...
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    <label className="mb-2 block text-sm font-bold uppercase tracking-[0.18em] text-slate-400">{t("Idioma das explicações")}</label>
                    <p className="mb-2 text-xs font-semibold text-slate-400">
                      {t("Idioma em que as traduções e explicações das aulas são escritas.")}
                    </p>
                    <div className="grid grid-cols-2 gap-2 sm:w-1/2">
                      {BASE_LANGUAGES.map((lang) => (
                        <button
                          key={lang.value}
                          type="button"
                          onClick={() => setForm((current) => ({ ...current, base_language: lang.value }))}
                          className={`flex flex-col items-center gap-1 rounded-[1.25rem] border-2 px-3 py-2.5 text-xs font-black transition ${
                            form.base_language === lang.value
                              ? 'border-primary bg-primary-light text-primary-dark'
                              : 'border-slate-200 bg-white text-slate-600 hover:border-primary'
                          }`}
                        >
          ...
          <aside className="space-y-6">
            ...
            <div className="app-surface border-primary/50 p-5 md:p-8">
              ...
              {generatorMessage ? (
                <p className={`mt-4 text-sm font-bold ${generatorTone === 'error' ? 'text-brand-pink' : 'text-primary-dark'}`}>
                  {generatorMessage}
                </p>
              ) : null}
              {generatedLesson ? (
