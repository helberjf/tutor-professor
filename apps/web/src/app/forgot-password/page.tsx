...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(cause instanceof ApiError ? cause.message : t("Não foi possível enviar o e-mail."));
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="mx-auto flex min-h-[70vh] w-full max-w-lg flex-col justify-center px-4 py-10">
      ...
      <div className="app-surface border-primary/40 p-6 md:p-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black text-slate-800 md:text-3xl">{t("Esqueci minha senha")}</h1>
        </div>
        ...
        {sent ? (
          <div className="mt-6 rounded-2xl bg-emerald-50 p-4">
            ...
            <p className="mt-2 text-sm font-semibold text-emerald-700">
              {t("O link vale por 1 hora. Confira também a caixa de spam.")}
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-6 grid gap-4">
            <p className="text-base leading-7 text-slate-600">
              {t("Informe o e-mail da conta. Enviaremos um link para você criar uma senha nova.")}
            </p>
            <label className="grid gap-2">
              <span className="text-sm font-black text-slate-600">{t("E-mail")}</span>
