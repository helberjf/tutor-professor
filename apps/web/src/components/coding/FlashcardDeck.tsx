  ...
  async function loadOverview(showLoading = false): Promise<boolean> {
    ...
    try {
      ...
      if (deckMountedRef.current && requestId === overviewLoadRequestRef.current) {
        setOverviewError(t("Não foi possível carregar o deck."));
      }
      return false;
    } finally {
  ...
  async function retryTopics(): Promise<boolean> {
    ...
    try {
      ...
      if (deckMountedRef.current && requestId === topicsLoadRequestRef.current) {
        setTopicsError(t("Não foi possível carregar os tópicos."));
      }
      return false;
    } finally {
    ...
    <div className="min-w-0 space-y-6">
      <section className="app-surface border-primary/30 p-6">
        <button type="button" onClick={onBack} className="mb-3 flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-primary">
          <ArrowLeft size={16} /> {t("Todas as matérias")}
        </button>
        <div className="flex items-center gap-3">
          <span className="text-3xl">{subjectIcon || '🃏'}</span>
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{t("Flashcards")}</p>
            <h1 className="text-2xl font-black text-slate-800">{subjectName}</h1>
          </div>
        </div>
        {stats && (
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatChip label={t("Novos")} value={stats.new} tone="sky" />
            <StatChip label={t("Aprendendo")} value={stats.learning} tone="amber" />
            <StatChip label={t("A revisar")} value={stats.review_due} tone="orange" />
            <StatChip label={t("Total")} value={stats.total} tone="slate" />
          </div>
        )}
      </section>

      <div className="study-mode-tabs">
        <DeckTab active={tab === 'study'} onClick={() => setTab('study')} icon={<Zap size={16} />} label={t("Estudar")} />
        <DeckTab active={tab === 'cards'} onClick={() => setTab('cards')} icon={<Layers size={16} />} label={t("Cards")} />
        <DeckTab active={tab === 'options'} onClick={() => setTab('options')} icon={<Settings2 size={16} />} label={t("Opções")} />
      </div>
      ...
      {overviewError && (
        <div role="alert" className="flex flex-col gap-2 rounded-2xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700 sm:flex-row sm:items-center sm:justify-between">
          <p>{overviewError}</p>
          <button type="button" onClick={() => void loadOverview(true)} className="rounded-xl border border-rose-200 bg-white px-3 py-1.5 text-xs hover:bg-rose-100">{t("Tentar recarregar deck")}</button>
        </div>
      )}
...
function StudyTab({ subjectId, subjectName, stats, onFinished, onLogged }: { subjectId: number; subjectName: string; stats?: DeckStats; onFinished: () => void; onLogged: () => void }) {
    ...
    return (
      <div className="rounded-3xl border-2 border-slate-100 bg-white px-6 py-12 text-center">
        <p className="text-lg font-black text-slate-800">
          {due > 0 ? `${due} card${due !== 1 ? 's' : ''} para estudar agora` : t("Nada para revisar agora 🎉")}
        </p>
        <p className="mt-1 text-sm text-slate-500">
          {due > 0 ? t("Erre / Difícil / Bom / Fácil — o intervalo da próxima revisão ajusta sozinho (FSRS).") : t("Volte mais tarde ou adicione novos cards.")}
        </p>
        {due > 0 && (
          <>
            <label className="mt-5 inline-flex cursor-pointer items-center gap-2 rounded-2xl border-2 border-slate-100 px-4 py-2 text-sm font-bold text-slate-600 hover:border-primary hover:text-primary">
              ...
              />
              <Keyboard size={16} /> {t("Modo decorar: digitar a resposta antes de ver")}
            </label>
            <div>
              <button
                type="button"
              ...
              >
                {loading ? <Loader2 size={18} className="animate-spin" /> : <Zap size={18} />} {t("Começar")}
              </button>
            </div>
          </>
        )}
  ...
  if (queue.length === 0) {
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-12 text-center">
        ...
        <h2 className="text-2xl font-black text-slate-800">{t("Sessão concluída!")}</h2>
        <div className="flex flex-wrap justify-center gap-6">
          <Tally label={t("Errei")} value={finalCounts.again} cls="text-rose-500" />
          <Tally label={t("Difícil")} value={finalCounts.hard} cls="text-amber-500" />
          <Tally label={t("Bom")} value={finalCounts.good} cls="text-sky-600" />
          <Tally label={t("Fácil")} value={finalCounts.easy} cls="text-emerald-600" />
        </div>
        <button type="button" onClick={start} disabled={loading} className="inline-flex items-center gap-2 rounded-2xl bg-primary-dark px-8 py-3 font-black text-white hover:bg-primary-dark disabled:opacity-50">
          {loading ? <Loader2 size={18} className="animate-spin" /> : <RotateCcw size={18} />} {t("Buscar mais")}
        </button>
      </div>
    );
  }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={t("Estudo de flashcards")}
    ...
    >
      <div className="relative flex h-[100dvh] w-full flex-col overflow-y-auto bg-white px-5 py-6 sm:h-auto sm:max-h-[92vh] sm:max-w-3xl sm:rounded-[1.5rem] sm:border-2 sm:border-slate-200 sm:p-7 sm:shadow-[0_28px_90px_rgba(15,23,42,0.35)]">
        <button
          ...
          aria-label={t("Fechar tela cheia dos flashcards")}
        ...
        <div className="space-y-5 pr-2 sm:pr-0">
          <div className="flex items-center justify-between gap-10">
            ...
            <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-black ${badge.cls}`}>{t(badge.label)}</span>
          </div>
            ...
            {!revealed && typingMode && (
              <div className="mt-5 space-y-3">
                <textarea
                  ...
                  rows={3}
                  aria-label={t("Escreva a resposta de memória")}
                  placeholder={t("Escreva a resposta de memória… (Enter compara, Shift+Enter quebra linha)")}
                ...
                {card.code_example && (
                  <p className="text-xs font-bold text-slate-400">
                    {t("A comparação usa só o verso — o código de exemplo não entra.")}
                  </p>
                )}
                ...
                {comparison && comparison.verdict !== 'exact' && (
                  ...
                  >
                    {comparison.verdict === 'close' ? t("Quase lá! Faltou pouco.") : t("Ainda não é isso.")}{' '}
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {comparison && comparison.verdict !== 'exact' ? (
                    ...
                    >
                      <RotateCcw size={18} /> {t("Tentar de novo")}
                    </button>
                  ) : (
                    <button
                      type="button"
                    ...
                    >
                      <CheckCircle2 size={18} /> {t("Comparar")}
                    </button>
                  )}
                  <button
                    type="button"
                  ...
                  >
                    <ChevronRight size={18} /> {t("Ver resposta")}
                  </button>
                </div>
              </div>
            )}
            ...
            {revealed && (
              <>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <GradeButton label={t("Errei")} preview={card.previews.again} onClick={() => answer('again')} disabled={submitting} suggested={suggested === 'again'} cls="border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" />
                  <GradeButton label={t("Difícil")} preview={card.previews.hard} onClick={() => answer('hard')} disabled={submitting} suggested={suggested === 'hard'} cls="border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" />
                  <GradeButton label={t("Bom")} preview={card.previews.good} onClick={() => answer('good')} disabled={submitting} suggested={suggested === 'good'} cls="border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100" />
                  <GradeButton label={t("Fácil")} preview={card.previews.easy} onClick={() => answer('easy')} disabled={submitting} suggested={suggested === 'easy'} cls="border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100" />
                </div>
                {suggested && (
                  <p className="mt-2 text-center text-xs font-bold text-slate-400">
                    {t("A comparação só sugere uma nota — quem decide é você.")}
                  </p>
                )}
              </>
            )}
  ...
  return (
    <div className="mt-4 rounded-2xl border-2 border-slate-100 p-4">
      <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-400">
        {t("Sua resposta ·")} {Math.round(comparison.similarity * 100)}% igual
      ...
      <p className="mt-2 text-[11px] font-bold text-slate-400">
        {t("verde = acertou · amarelo = faltou · riscado = sobrou")}
      </p>
    </div>
  );
}
...
function CardsTab({ subjectId, subjectName, overview, topics, topicsError, topicsLoading, retryTopics, onReload, onChanged }: {
  ...
  async function generateWithAi() {
    ...
    try {
      if (!selectedTopicId) {
        setAiError(t("Selecione um tópico."));
        return;
      }
      ...
      if (!overviewReloaded) {
        setAiError(t("As questões foram criadas, mas não foi possível recarregar o deck."));
        return;
      }
      ...
      if (mountedRef.current) {
        setAiError(err instanceof Error ? err.message : t("Não foi possível criar as questões."));
      }
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        >
          <Sparkles size={18} /> {t("Criar com IA")}
        </button>
      </div>
      ...
      {creatingWithAi && (
        <div aria-busy={generatingWithAi} className="space-y-3 rounded-2xl border-2 border-primary/30 bg-primary-light/30 p-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-black text-slate-700">{t("Criar cards com IA")}</p>
            <button
              type="button"
              onClick={() => setCreatingWithAi(false)}
              disabled={generatingWithAi}
              aria-label={t("Cancelar criação com IA")}
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-400">{t("Tópico *")}</span>
            <select
              required
            ...
            >
              <option value="">{t("Selecione um tópico")}</option>
              {topics.map((topic) => <option key={topic.id} value={topic.id}>{topic.title}</option>)}
            </select>
            {topics.length === 0 && !topicsError && (
              <span role="status" className="text-xs font-bold text-amber-700">{t("Nenhum tópico disponível para gerar questões.")}</span>
            )}
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-400">{t("Contexto opcional")}</span>
            <textarea
              value={context}
              ...
              rows={3}
              disabled={generatingWithAi}
              placeholder={t("Ex.: foque em exemplos práticos e erros comuns")}
          ...
          <p className="text-xs font-bold text-slate-500">{t("Serão criadas 5 questões")}</p>
          ...
          >
            ...
            {generatingWithAi ? 'Criando...' : t("Criar 5 questões")}
          </button>
        </div>
      )}
      ...
      {topicsError && (
        <div role="alert" className="flex flex-col gap-2 rounded-xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            {topicsLoading ? 'Recarregando...' : t("Tentar recarregar tópicos")}
          </button>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-white px-6 py-12 text-center">
          <p className="font-bold text-slate-500">{cards.length === 0 ? t("Nenhum card ainda.") : t("Nenhum card encontrado.")}</p>
          <p className="mt-1 text-sm text-slate-400">{cards.length === 0 ? t("Crie cards manualmente ou gere tópicos com IA na aba Estudar da matéria.") : t("Tente outra busca.")}</p>
        </div>
      ) : (
    ...
    <div className="flex min-w-0 flex-col gap-3 rounded-2xl border-2 border-slate-100 bg-white px-4 py-3 sm:flex-row sm:items-start">
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2 text-[11px] font-bold">
          <span className={`rounded-full px-2 py-0.5 ${badge.cls}`}>{t(badge.label)}</span>
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-500">{card.topic_title}</span>
          <span className="text-slate-400">{t("próx.:")} {card.interval_label}</span>
      ...
      <div className="flex shrink-0 justify-end gap-1 sm:justify-start">
        <button type="button" aria-label={t("Editar card")} onClick={() => setEditing(true)} disabled={locked} className="rounded-xl p-1.5 text-slate-400 hover:bg-slate-100 hover:text-primary disabled:opacity-50"><Pencil size={15} /></button>
        <button type="button" aria-label={t("Excluir card")} onClick={remove} disabled={busy || locked} className="rounded-xl p-1.5 text-slate-300 hover:bg-rose-50 hover:text-rose-500 disabled:opacity-50"><Trash2 size={15} /></button>
      </div>
    </div>
  );
}
...
function CardForm({ subjectId, initial, disabled = false, onCancel, onSaved }: { subjectId?: number; initial?: DeckCard; disabled?: boolean; onCancel: () => void; onSaved: () => void }) {
  ...
  async function save() {
    if (disabled) return;
    if (!front.trim() || !back.trim()) { setErr(t("Frente e verso são obrigatórios.")); return; }
    setBusy(true);
    setErr('');
    try {
      if (initial) {
      ...
      setErr(t("Não foi possível salvar o card."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-3 rounded-2xl border-2 border-primary/30 bg-primary-light/30 p-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-black text-slate-700">{initial ? t("Editar card") : t("Novo card")}</p>
        <button type="button" aria-label={t("Fechar formulário do card")} onClick={onCancel} disabled={disabled || busy} className="rounded-lg p-1 text-slate-400 hover:bg-white disabled:opacity-50"><X size={16} /></button>
      </div>
      <input
...
function OptionsTab({ subjectId, config, onSaved }: { subjectId: number; config?: DeckConfig; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      setMsg(t("Opções salvas."));
      onSaved();
    } catch {
      setMsg(t("Não foi possível salvar as opções."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-5">
      <Group title={t("Limites diários")} hint={t("Quantos cards inéditos e revisões entram por dia.")}>
        <NumField label={t("Novos cards / dia")} value={form.new_per_day} onChange={(v) => set('new_per_day', v)} />
        <NumField label={t("Máx. revisões / dia")} value={form.max_reviews_per_day} onChange={(v) => set('max_reviews_per_day', v)} />
        <Toggle label={t("Novos cards ignoram o limite de revisões")} value={form.new_cards_ignore_review_limit} onChange={(v) => set('new_cards_ignore_review_limit', v)} />
      </Group>

      <Group title={t("Cards novos")} hint={t("Passos (minutos) antes do card 'graduar' para revisão e a ordem de entrada.")}>
        <TextField label={t("Passos de aprendizado")} value={form.learning_steps} onChange={(v) => set('learning_steps', v)} placeholder="1 10" />
        <SelectField label={t("Ordem de inserção")} value={form.insertion_order} onChange={(v) => set('insertion_order', v as DeckConfig['insertion_order'])} options={[['sequential', 'Sequencial'], ['random', 'Aleatória']]} />
        <NumField label={t("Intervalo de graduação (dias)")} value={form.graduating_interval} onChange={(v) => set('graduating_interval', v)} />
        <NumField label={t("Intervalo fácil (dias)")} value={form.easy_interval} onChange={(v) => set('easy_interval', v)} />
      </Group>

      <Group title={t("Lapsos")} hint={t("O que acontece quando você erra e quando um card vira 'leech' (erra demais).")}>
        <TextField label={t("Passos de reaprendizado")} value={form.relearning_steps} onChange={(v) => set('relearning_steps', v)} placeholder="10" />
        <NumField label={t("Limite de leech (lapsos)")} value={form.leech_threshold} onChange={(v) => set('leech_threshold', v)} />
        <SelectField label={t("Ação no leech")} value={form.leech_action} onChange={(v) => set('leech_action', v as DeckConfig['leech_action'])} options={[['tag', t("Apenas marcar")], ['suspend', 'Suspender']]} />
      </Group>

      <Group title="FSRS" hint={t("Algoritmo moderno do Anki. Maior retenção = revisões mais frequentes.")}>
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold uppercase tracking-wide text-slate-400">{t("Retenção desejada —")} {Math.round(form.desired_retention * 100)}%</label>
          <input aria-label={t("Retenção desejada")} type="range" min={70} max={99} value={Math.round(form.desired_retention * 100)} onChange={(e) => set('desired_retention', Number(e.target.value) / 100)} className="accent-primary" />
        </div>
        <NumField label={t("Intervalo máximo (dias)")} value={form.maximum_interval} onChange={(v) => set('maximum_interval', v)} />
        <div className="sm:col-span-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-400">{t("Parâmetros FSRS (19 pesos)")}</span>
            <button type="button" onClick={() => set('fsrs_parameters', '')} className="text-[11px] font-bold text-primary hover:underline">{t("Restaurar padrão")}</button>
          </div>
          <textarea
            value={form.fsrs_parameters}
            onChange={(e) => set('fsrs_parameters', e.target.value)}
            rows={3}
            placeholder={t("deixe em branco para usar os pesos padrão")}
          ...
          />
          <p className="mt-1 text-[11px] text-slate-400">{t("19 valores separados por vírgula. Em branco = padrão do Anki. Valores inválidos são ignorados (usa-se o padrão).")}</p>
        </div>
      </Group>
      ...
      <button type="button" onClick={save} disabled={busy} className="flex items-center justify-center gap-2 rounded-2xl bg-primary-dark px-8 py-3 font-black text-white hover:bg-primary-dark disabled:opacity-50">
        {busy ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} {t("Salvar opções")}
      </button>
    </div>
  );
}
