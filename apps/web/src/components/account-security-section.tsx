...
export function AccountSecuritySection() {
  ...
  async function changePassword(event: React.FormEvent) {
    ...
    try {
      ...
      setError(cause instanceof ApiError ? cause.message : t("Não foi possível trocar a senha."));
      setBusy(null);
    }
  }
  ...
  async function revokeSessions() {
    ...
    try {
      ...
      setError(cause instanceof ApiError ? cause.message : t("Não foi possível encerrar as sessões."));
      setBusy(null);
    }
  }
  ...
  return (
    <section className="app-surface mb-6 border-slate-200 p-5 md:p-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-slate-400">{t("Segurança")}</p>
          <h2 className="text-2xl font-black text-slate-800 md:text-3xl">{t("Senha e acessos")}</h2>
        </div>
      </div>
      ...
      <form onSubmit={changePassword} className="mt-6 grid gap-4">
        <label className="grid gap-2">
          <span className="text-sm font-black text-slate-600">{t("Senha atual")}</span>
          <input
            type="password"
            required
        ...
        <p className="text-sm font-semibold text-slate-500">
          {t("Trocar a senha encerra todas as sessões, inclusive esta.")}
        </p>
        <button
          type="submit"
          disabled={busy !== null}
