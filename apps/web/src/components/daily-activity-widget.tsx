    ...
    return (
      <div className="rounded-xl border-2 border-slate-200 bg-slate-50 p-4">
        <p className="text-center text-sm font-medium text-slate-600">
          {t("Nenhuma atividade hoje. Comece a estudar! 📚")}
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-xl border-2 border-primary bg-white p-4 shadow-sm">
      ...
      <div className="mb-3 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-2 py-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{t("Eventos")}</p>
        </div>
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-2 py-2">
          <p className="flex items-center justify-center gap-1 text-base font-black text-slate-800"><Clock size={13} /> {durationLabel}</p>
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{t("Tempo registrado")}</p>
        </div>
        <div className="rounded-lg border border-slate-100 bg-slate-50 px-2 py-2">
          <p className="text-base font-black text-slate-800">{scoreLabel}</p>
          <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{t("Média c/ nota")}</p>
        </div>
      </div>
      ...
      <div className="mt-3 border-t border-slate-200 pt-2">
        <p className="text-center text-xs text-slate-600">
          <strong>{activities.total_activities}</strong> {t("atividades registradas hoje")}
        </p>
        {updatedAt && (
          <p className="mt-1 flex items-center justify-center gap-1 text-[10px] font-medium text-slate-400">
            <RefreshCw size={10} /> {t("Atualizado às")} {updatedAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
          </p>
        )}
      </div>
    </div>
