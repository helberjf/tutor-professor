    ...
    <div className="space-y-5">
      ...
      <div className="rounded-[1.4rem] border-2 border-emerald-100 bg-emerald-100/70 p-5 dark:border-emerald-300/30 dark:bg-emerald-400/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.14em] text-emerald-600 dark:text-emerald-200">{t("Atividade por período")}</p>
            <h2 className="mt-1 text-xl font-black text-slate-800 dark:text-slate-50">{t("O que você fez ·")} {currentPeriodLabel}</h2>
      ...
      <div className="flex flex-col gap-2 rounded-[1.4rem] border-2 border-sky-100 bg-sky-100/70 p-4 dark:border-sky-300/30 dark:bg-sky-400/10 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-sky-600 dark:text-sky-100">{t("Comparativo semanal")}</p>
          <p className="mt-1 text-sm font-bold text-slate-700 dark:text-slate-100">{thisWeekActivities} {t("atividades nos 7 últimos dias da janela ·")} {previousWeekActivities} nos 7 anteriores</p>
        </div>
        <span className={`w-fit rounded-full px-3 py-1 text-sm font-black ${weeklyDelta >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-100' : 'bg-rose-100 text-rose-700 dark:bg-rose-400/15 dark:text-rose-100'}`}>
          {previousWeekActivities === 0 ? t("Primeira semana") : `${weeklyDelta >= 0 ? '+' : ''}${weeklyDelta} eventos`}
        </span>
      </div>
      ...
      <div className="rounded-[1.4rem] border-2 border-amber-100 bg-white/90 p-5">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-black text-slate-800">{t("Acertos e erros do Modo questões")}</h2>
          </div>
          <span className="inline-flex w-fit items-center gap-2 rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-700">
            <ClipboardList size={14} /> {questionMetrics.length} {t("matéria")}{questionMetrics.length === 1 ? '' : 's'}
          </span>
        </div>

        {questionMetrics.length === 0 ? (
          <p className="rounded-2xl bg-slate-50 px-4 py-4 text-sm font-bold text-slate-500">
            {t("Ainda não há questões respondidas. Abra uma matéria em Programação, entre em Modo questões e resolva algumas para preencher este painel.")}
          </p>
        ) : (
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-[1.4rem] border-2 border-slate-100 bg-white/90 px-4 py-3">
        <p className="mr-auto text-sm font-black text-slate-700">
          {t("Janela de 30 dias ·")} <span className="font-bold text-slate-500">{formatDateLabel(windowStart)} {t("até")} {formatDateLabel(windowEnd)}</span>
        </p>
        <button
          type="button"
        ...
        >
          <ChevronLeft size={16} /> {t("30 dias antes")}
        </button>
        <button
          type="button"
        ...
        >
          {t("30 dias depois")} <ChevronRight size={16} />
        </button>
        {windowOffset === 0 ? null : (
          <button
            type="button"
      ...
      <div className="rounded-[1.4rem] border-2 border-slate-100 bg-white/90 p-5">
        <p className="mb-4 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{t("Pomodoros —")} {windowLabel}</p>
      ...
      <div className="rounded-[1.4rem] border-2 border-slate-100 bg-white/90 p-5">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{t("Atividade —")} {windowLabel}</p>
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-500">
          <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-[3px] bg-emerald-400" /> {t("Atividade registrada")}</span>
          <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-[3px] bg-sky-300" /> {t("Só pomodoro")}</span>
          <span className="flex items-center gap-1.5"><span className="inline-block h-3 w-3 rounded-[3px] bg-slate-100 border border-slate-200" /> {t("Sem atividade")}</span>
        </div>
      </div>

      <div className="rounded-[1.4rem] border-2 border-slate-100 bg-white/90 p-5">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">{t("Histórico recente")}</p>
        ...
        {dashboard?.last_study_date && (
          <p className="mt-3 text-xs text-slate-400">
            {t("Último estudo registrado:")} <span className="font-bold">{formatDateLabel(dashboard.last_study_date)}</span>
          </p>
        )}
      </div>
    </div>
