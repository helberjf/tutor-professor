...
export function AccountDataSection() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      setError(cause instanceof ApiError ? cause.message : t("Não foi possível exportar os dados."));
    } finally {
      setBusy(null);
    }
  }
    ...
    try {
      ...
      setError(
        cause instanceof ApiError
          ...
          : t("Não foi possível apagar a conta."),
      );
      setBusy(null);
    }
  }
  ...
  return (
    <section className="app-surface mb-6 border-rose-200 p-5 md:p-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-slate-400">{t("Seus dados")}</p>
          <h2 className="text-2xl font-black text-slate-800 md:text-3xl">{t("Exportar ou apagar")}</h2>
        </div>
      </div>
      ...
      <p className="mt-4 text-sm leading-6 text-slate-500">
        {t("Baixe uma copia de tudo que guardamos sobre sua conta e sobre os perfis dos estudantes. A senha e a chave de IA nunca entram no arquivo.")}
      </p>
      <button
        type="button"
      ...
      <div className="mt-8 rounded-[1.25rem] border-2 border-rose-100 bg-rose-50/50 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-lg font-black text-rose-800">{t("Apagar a conta")}</p>
            <p className="mt-1 text-sm font-semibold text-rose-700">
              {t("Remove a conta, os perfis de estudante e todo o histórico de estudo. Não dá para desfazer. Exporte seus dados antes, se quiser guardá-los.")}
            </p>
          </div>
        </div>
        ...
        {confirming ? (
          <form onSubmit={handleDelete} className="mt-4 grid gap-3">
            <label className="grid gap-2">
              <span className="text-sm font-black text-rose-800">{t("Confirme sua senha")}</span>
              <input
                type="password"
                required
