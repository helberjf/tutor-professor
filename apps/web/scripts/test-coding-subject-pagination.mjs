...
assert.match(curriculum, /curriculum\.getCodingSubjectPage/, 'the curriculum requests paged subjects');
assert.doesNotMatch(curriculum, /curriculum\.getCodingSubjects\(\)/, 'the curriculum no longer downloads every subject');
...
assert.match(curriculum, /curriculum\.markCodingSubjectUsed/, 'opening a subject records last use');
assert.match(
  curriculum,
  /session\.total_due[\s\S]*curriculum\.markCodingSubjectUsed\(subject\.id\)[\s\S]*setView\(\{ type: 'review'/,
...
assert.match(api, /\$\{base\}\/subjects\/page/, 'the API client uses the page endpoint');
